...
def prop_perf(v, T):
    ...
    return 0.5 * T * v * (np.sqrt(T * 2 / (A_prop * v**2 * const.rho0) + 1) + 1)
...
if __name__ == "__main__":
    S= 10
    v_range = np.linspace(0.1, 5, 5)
    alpha = np.deg2rad(np.linspace(-10, 15, 26))
    ...
    for i, v in enumerate(v_range):
        ...
        P[i, :] = np.where(Tp_req>0, Pp + Pr, np.inf)
