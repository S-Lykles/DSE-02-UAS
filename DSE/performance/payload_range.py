...
def Wf_payload_mission(CL, CD, S, eta_cruise, eta_loiter, SFC, P_max, P_aux, t_takeoff=4*60, h_cruise1=500, h_cruise2=500, W0=const.MTOW, drop_payload=0, R=None):
    if R is None:
    ...
    W = np.diag(Wf_range(W, R, SFC, eta_cruise, CL, CD, S, P_extra, h=h_cruise1))
    W = Wf_loiter(W, [const.T_loiter_pay], SFC, eta_loiter, CL, CD, S, P_extra)[:,-1]
    W -= drop_payload * const.g0
    W -= Wf_climb(h_cruise2, W, SFC)
    W = np.diag(Wf_range(W, R, SFC, eta_cruise, CL, CD, S, P_extra, h=h_cruise2, v_cruise=0))
    
    return R, W0 - (W + drop_payload * const.g0)


def T_endurance_mission(Wf_max, CL, CD, S, eta_cruise, eta_loiter, SFC, P_max, P_aux, t_takeoff=4*60, h_cruise1=500, h_cruise2=500, W0: float=const.MTOW, R=None):
    N = 30
    if R is None:
    ...
    W -= Wf_take_off(P_max, SFC, t_takeoff)
    W -= Wf_climb(h_cruise1, W, SFC)
    W = Wf_range(W, R, SFC, eta_cruise, CL, CD, S, P_extra, h=h_cruise1)[0]
    t = np.linspace(0, 2.5*const.T_loiter_end, N)
    W = Wf_loiter(W, t, SFC, eta_loiter, CL, CD, S, P_extra)
    W = W.flatten()
    W -= Wf_climb(max(h_cruise2-const.h_loiter,0), W, SFC)
    W = Wf_range(W, R, SFC, eta_cruise, CL, CD, S, P_extra, h=h_cruise2, v_cruise=0)
    i = np.arange(N)
...
def payload_range_diagram(OEW:float, Wf_max, Payload_max, CL, CD, S, eta_cruise, eta_loiter, SFC, P_max, P_aux, t_takeoff=4*60, h_cruise1=500, h_cruise2=500, W0:float=const.MTOW, drop_payload=False):
    if drop_payload == True:
        raise NotImplementedError
    
    Rp = np.linspace(0, 2*const.R_cruise, 40)
    Rp, Wf = Wf_payload_mission(CL, CD, S, eta_cruise, eta_loiter, SFC, P_max, P_aux, t_takeoff, h_cruise1, h_cruise2, W0=W0, R=Rp)
    ...
    R2, Wf2 = Wf_payload_mission(CL, CD, S, eta_cruise, eta_loiter, SFC, P_max, P_aux, t_takeoff, h_cruise1, h_cruise2, W0=W0, R=R2)
