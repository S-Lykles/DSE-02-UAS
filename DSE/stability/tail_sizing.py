...
def vertical_tail(CL_h=aero_constants.Cl_cruise_h,AR_h=6.8,bh=2.3,Xcg=extreme_cg_calc(),l_fus=2,eta=0.95,b_max=0.7,b=aero_constants.b,S=aero_constants.S,CL=aero_constants.CL_cruise,Cl_alpha_v=aero_constants.Cl_alpha_v,deg2rad=const.deg2rad):
            ...
            for p in range(len(taper_v)):
                ...
                for r in range(len(A)):
                    ...
                    for q in range(iteration):
                        bv = np.sqrt(AR_v[i] * Sv)
                        if bv < min_height:
                            bv = min_height
                        Cv_r = 2 / (1 + taper_v[p]) * (Sv / bv)
                        Cv_bar = 2 / 3 * Cv_r * ((1 + taper_v[p] + taper_v[p] ** 2) / (1 + taper_v[p]))
                        Cv_tip = taper_v[p] * Cv_r

                        sweep_v_TE = np.arctan((0.75 * (Cv_r - Cv_tip)) / (0.5 * bv))
                        bb = np.sin(sweep_v_TE) * bv
                        MAX_1 = A[r] + Cv_r + bb

                        # Updated values
                        X_LEMAC_v = bv / 6 * ((1 + 2 * taper_v[p]) / (1 + taper_v[p])) * np.tan(sweep_v[j] * deg2rad)
                        lv = l_max - max(A) + A[r] - Xcg + X_LEMAC_v + 0.25 * Cv_bar
                        sweep_05_cord_v = sweep_v[j]
                        CL_v_alpha = (Cl_alpha_v * AR_v[i]) / (2 + np.sqrt(4 + (((AR_v[i] * Beta) / eta) ** 2) * (
                                    ((np.tan(sweep_05_cord_v * deg2rad)) ** 2 / Beta ** 2) + 1)))
                        ...
                        if Sv < 0:
                            Sv = Sv_prev
                    MAX_1_r.append(MAX_1)
                    Surface_r.append(Sv)
                    Span_r.append(bv)
                    Moment_arm_r.append(lv)
                    root_cord_r.append(Cv_r)
                MAX_1_p.append(MAX_1_r)
                Surface_p.append(Surface_r)
                Span_p.append(Span_r)
                Moment_arm_p.append(Moment_arm_r)
                root_cord_p.append(root_cord_r)
            MAX_1_j.append(MAX_1_p)
            Surface_j.append(Surface_p)
            Span_j.append(Span_p)
            Moment_arm_j.append(Moment_arm_p)
            root_cord_j.append(root_cord_p)
        MAX.append(MAX_1_j)
        Surface.append(Surface_j)
        Span.append(Span_j)
        Moment_arm.append(Moment_arm_j)
        root_cord.append(root_cord_j)
    ...
    optimal_surface_v = Surface[i_opt][j_opt][p_opt][r_opt]
    optimal_span_v = Span[i_opt][j_opt][p_opt][r_opt]
    optimal_moment_arm = Moment_arm[i_opt][j_opt][p_opt][r_opt]
    optimal_root_cord = root_cord[i_opt][j_opt][p_opt][r_opt]
    optimal_sweep_v = sweep_v[j_opt]
    optimal_AR_v = AR_v[i_opt]
    optimal_taper = taper_v[p_opt]
    optimal_MAX_1 = MAX[i_opt][j_opt][p_opt][r_opt]
    ...
    if PRINT == True:
        ...
        print('taper ratio =', optimal_taper)
        print('MAX_1 =', optimal_MAX_1)
        print('-----------------------------------------------------------------')
