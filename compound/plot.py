...
if __name__ == '__main__':
    ...
    #     plot_fuel(np.linspace(2,6.3,n), np.linspace(1.5,5,n), lambda b, S: dragpolar_comp(b, S, CL_start=0.01), b_design=b_design, S_design=S_design, which=which,name='compound_mf_'+which+'.pdf')

    # plot_cd0(np.linspace(2,6.3,n), np.linspace(1.5,5,n), lambda b, S: dragpolar_comp(b, S, CL_start=0., CL_step=1)[1][0])

    plot_power_curves([160, 230, 280], [(5, 3.76), (4.5, 3.5), (4, 3.2), (3, 2.7)], 1, lambda b, S: dragpolar_comp(b, S, CL_start=0.01, CL_end=CL_max), CD0, S_design, name='compound_pc.pgf')
